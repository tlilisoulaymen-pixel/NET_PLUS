...
const NAV = [
  ...
  { v: "pointage", fab: true, isAction: true, icon: I.pin },
  { v: "scan", label: "Scanner", icon: I.camera },
  { v: "alerts", label: "Alertes", icon: I.bell },
];

function navHTML() {
  return `<nav class="bottom-nav">` + NAV.map(n => n.fab
    ? `<button class="fab" ${n.isAction ? 'data-action="checkin"' : `data-nav="${n.v}"`} aria-label="${n.label || 'Action'}">${n.icon}</button>`
...
V.home = () => {
  ...
  const themeIcon = document.documentElement.dataset.theme === "light" ? "🌙" : "☀️";
  const themeToggle = `<button class="theme-toggle" id="themeToggleBtn">${themeIcon}</button>`;
  ...
  return `<div class="screen">
    <div class="greeting">
      <div class="avatar" id="profileUploadBtn">
        ${avatar}
        <input type="file" id="profileUploadInput" accept="image/*" hidden>
      </div>
      <div style="flex-grow:1; cursor:pointer" data-nav="profile">
        <div class="hello">Bonjour 👋</div>
        <div class="name">${esc(b.profile.full_name)}</div>
      </div>
      ${score}
      ${themeToggle}
    </div>
...
document.addEventListener("click", async ev => {
  const t = ev.target.closest("[data-nav],[data-go],[data-back],[data-type],[data-sev],[data-rm],[data-action],#checkinBtn,#checkoutBtn,#confirmCheckout,#anomalyBtn,#addPhoto,#sendAnomaly,#moreHist,#logoutBtn,#manualGo,#skipScan,#simGpsBtn,#profileUploadBtn,#themeToggleBtn");
  if (!t) return;

  if (t.id === "themeToggleBtn") {
    const isLight = document.documentElement.dataset.theme === "light";
    const newTheme = isLight ? "dark" : "light";
    document.documentElement.dataset.theme = newTheme;
    localStorage.setItem("netplus_theme", newTheme);
    t.textContent = newTheme === "light" ? "🌙" : "☀️";
    return;
  }

  if (t.dataset.action === "checkin") {
    if (S.boot.active) return navigate("active");
    const upcoming = S.boot.missions.filter(m => !m.checked_out);
    if (upcoming.length > 0) return navigate("pointage", { task: upcoming[0].task });
    return toast("AUCUNE MISSION POUR LE MOMENT");
  }

  if (t.id === "profileUploadBtn") return document.getElementById("profileUploadInput").click();
...
document.addEventListener("change", async ev => {
  if (ev.target.id === "profileUploadInput" && ev.target.files.length) {
    try {
      const close = overlay("Mise à jour photo…");
      const f = ev.target.files[0];
      const formData = new FormData();
      formData.append("file", f);
      formData.append("is_private", "0");
      formData.append("folder", "Home/Attachments");
      if (S.boot.employee_id) {
        formData.append("doctype", "Employee");
        formData.append("docname", S.boot.employee_id);
        formData.append("fieldname", "image");
      }
      const res = await fetch("/api/method/upload_file", {
        method: "POST", headers: { "X-Frappe-CSRF-Token": CFG.csrf }, body: formData
      });
      const data = await res.json();
      close();
      if (!res.ok) throw new Error("Échec upload");
      S.boot.profile.image = data.message.file_url;
      render();
      toast("Photo de profil mise à jour !");
    } catch(e) { toast("Erreur photo de profil", true); }
    return;
  }
...
/* ------------------------------ démarrage ------------------------------ */
(async function init() {
  const savedTheme = localStorage.getItem("netplus_theme") || "dark";
